...
class GlideClient {
    ...
    func magicAuth(startVerificationDto: StartVerificationDto) async throws -> StartVerificationResponseDto {
        let url = URL(string: "\(config.internalApiBaseUrl)/magic-auth/verification/start")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        request.addValue(getBasicAuthHeader(), forHTTPHeaderField: "Authorization")
    ...
    func verifyToken(checkCodeDto: CheckCodeDto) async throws -> Bool {
        ...
        let resData = try JSONDecoder().decode([String: Bool].self, from: data)
        return resData["verified"] ?? false
    }
}
